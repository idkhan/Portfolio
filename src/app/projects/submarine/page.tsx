...
function Model() {
  const { scene } = useGLTF("/submarine.glb"); // Replace with your model path
  return <primitive object={scene} scale={10} position={[0, 0, -1.5]} />;
}
    ...
    <div className="relative scroll-smooth overflow-y-scroll md:snap-proximity md:snap-y h-screen font-Poppins transition-all duration-500 pointer-events-none">
      ...
      <div className="absolute inset-0 -z-10 bg-white">
        <Canvas camera={{ position: [0, 10, 0], fov: 40 }}>
          ...
          <directionalLight position={[5, 0, 0]} intensity={2} color={"white"} />
          <directionalLight position={[-5, 0, 0]} intensity={2} color={"white"} />
          <Model />
